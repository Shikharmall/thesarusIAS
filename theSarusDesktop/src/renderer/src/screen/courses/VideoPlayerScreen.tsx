...
import type { Video } from "../../utils/types/courses";
    ...
    return (
        <div className="flex flex-col lg:flex-row h-screen bg-gray-100">
            ...
            <div className="flex-1 p-4">
                ...
                <button
                    onClick={() => navigate(-1)}
                    className="flex items-center gap-2 text-white hover:text-[#eee] mb-4 transition fixed top-6 left-6 z-50 px-3 py-2 rounded-full cursor-pointer shadow-md"
            ...
            <div className="w-full lg:w-80 bg-white border-l border-gray-200 p-4 overflow-y-auto">
                ...
                <ul className="space-y-3">
                    {videos.map((video) => (
                        <li
                            key={video.id}
                            className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition border-l-4 
                ${currentVideo.id === video.id
                        ...
                        >
                            ...
                            <Play
                                size={18}
                                className={`${currentVideo.id === video.id
                                    ? "text-blue-600"
                                    : "text-gray-400"
                                    }`}
                            />
